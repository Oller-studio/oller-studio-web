...
const PRINT_REQUEST_CONFIRMATION_TEMPLATE_KEY = "print_request_confirmation";

export function PrintRequestConfirmationEditor({
  initialSubject,
  initialMessage,
}: {
  ...
  async function save() {
    ...
    const res = await fetch("/api/admin/email-templates", {
      ...
      body: JSON.stringify({ key: PRINT_REQUEST_CONFIRMATION_TEMPLATE_KEY, subject, message }),
    }).catch(() => null);
  ...
  return (
    <div className="flex w-full max-w-xl flex-col gap-3 rounded-xl border border-border bg-background p-4 shadow-sm">
      <p className="text-sm font-semibold uppercase tracking-wide">Print request confirmation</p>
      <p className="text-xs text-muted">
        Sent the moment someone requests a sold-out print-on-demand color (not a Limited Edition —
        those use Notify Me / Restock instead). Use {"{{product}}"} and {"{{color}}"} anywhere
        below. The team also gets an internal notification for the same request, under Print
        request.
      </p>
