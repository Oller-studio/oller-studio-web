...
const COLLAB_CONFIRMATION_TEMPLATE_KEY = "collab_confirmation";

export function CollabConfirmationEditor({
  initialSubject,
  initialMessage,
}: {
  ...
  async function save() {
    ...
    const res = await fetch("/api/admin/email-templates", {
      ...
      body: JSON.stringify({ key: COLLAB_CONFIRMATION_TEMPLATE_KEY, subject, message }),
    }).catch(() => null);
  ...
  return (
    <div className="flex w-full max-w-xl flex-col gap-3 rounded-xl border border-border bg-background p-4 shadow-sm">
      <p className="text-sm font-semibold uppercase tracking-wide">Collab confirmation</p>
      <p className="text-xs text-muted">
        Sent when someone submits the Collab-with-us form.
      </p>
