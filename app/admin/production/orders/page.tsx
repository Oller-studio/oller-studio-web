...
export default async function AdminProductionOrdersPage() {
  ...
  const boxGroups = QUEUE_BOXES.map((box) => {
    const orders = queue.filter(
      (o) => boxForStage(o.fulfillmentStatus) === box,
    );
    ...
    <div className="flex flex-col gap-4">
      <div>
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-display text-3xl font-semibold">Paid orders</h1>
        </div>
        <p className="text-sm text-muted">
          Paid orders not yet shipped, oldest first — what actually needs doing
          today.
        </p>
      </div>
            ...
            <div className="rounded-t-xl bg-border/20 px-4 py-2">
              <div>
                <span className="text-sm font-semibold">
                  {FULFILLMENT_LABELS[g.box]}
                </span>
                <span className="ml-2 text-xs text-muted">({g.count})</span>
              </div>
              {g.items && (
                <p className="truncate text-xs text-muted">{g.items}</p>
              )}
            </div>
            {g.orders.length === 0 ? (
                    ...
                    >
                      ...
                      {g.box === "NEW_ORDER" ? (
                        <PrintingControl
                          orderId={o.id}
                          startedPrintingAt={
                            o.startedPrintingAt?.toISOString() ?? null
                          }
                          printMinutes={
                            printMinutesBySlug[o.items[0]?.colorwaySlug] ?? null
                          }
                        />
                      ) : (
                        <AdvanceStageButton
